...
const SECTIONS = [
  ...
  },
  {
    ...
    pages: [
      ...
      { path: '/sign-up', desc: 'Sign up (role param)' },
      { path: '/onboarding', desc: 'Role redirect gateway' },
    ]
  },
  {
    label: 'Caregiver — login required',
    color: '#0F766E',
    bg: '#F0FDFA',
    border: '#99F6E4',
    pages: [
      ...
      { path: '/settings/data-rights', desc: 'Export / delete' },
      { path: '/id/[caregiverId]', desc: 'Wallet ID card' },
      { path: '/verify/[slug]', desc: 'Public verification' },
    ]
  },
  {
    label: 'Agency — approved login required',
    color: '#1D4ED8',
    bg: '#EFF6FF',
    border: '#BFDBFE',
    pages: [
      ...
      { path: '/agency/clients/[id]', desc: 'Match analysis + gaps' },
      { path: '/agency/airecruit', desc: 'AIRecruit hub' },
      { path: '/agency/airecruit/new', desc: 'New campaign' },
      { path: '/agency/airecruit/[id]', desc: 'Campaign + calls' },
  ...
  {
    label: 'Admin — super access only',
    color: '#B91C1C',
    bg: '#FEF2F2',
    border: '#FECACA',
    pages: [
      { path: '/admin', desc: 'Dashboard' },
      { path: '/admin/agencies', desc: 'Approve / reject agencies' },
    ...
    <div style={{ minHeight: '100vh', background: '#F7F4F0', fontFamily: S, padding: '32px 24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 13, color: '#64748B', margin: '0 0 32px' }}>
          All {SECTIONS.reduce((a, s) => a + s.pages.length, 0)} pages — grouped by access level. Click any path to open it.
        </p>
            ...
            <div key={section.label} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
                {section.pages.map((page, i) => (
                  ...
                  >
                    <div style={{ fontSize: 13, fontWeight: 700, color: section.color, fontFamily: 'monospace', marginBottom: 3 }}>
                      {page.path}
                    </div>
